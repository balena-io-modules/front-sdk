...
describe('Topics', function () {
	...
	it('should return an empty results list for an invalid topic', function () {
		return this.globals.front.topic
			.listConversations({ topic_id: 'top_xxxxx' })
			.then(() => {
	...
	it.skip('should list all of conversations associated with a topic', function () {
		return githubInst.issues
			...
			.then((issue) => {
				...
				return this.globals.front.topic.listConversations({
					topic_id: topicId,
				});
			})
			.then((response: TopicConversations) => {
				...
				conversation.last_message.body.should.eq(
					this.global.testMessageResponse,
				);
