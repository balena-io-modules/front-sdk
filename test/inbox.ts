...
describe('Inboxes', function () {
	...
	it('should get known test inbox', function () {
		return this.globals.front.inbox
			.get({
			...
			.then((response: Inbox) => {
				...
				response._links.self.should.startsWith(
					`${apiUrl}/inboxes/${this.globals.inbox.id}`,
				);
				response._links.related.should.include.keys(
					'channels',
					'conversations',
					'teammates',
				);
	...
	it('should list all channels for an inbox, and find integration test channel', function () {
		return this.globals.front.inbox
			.listChannels({ inbox_id: this.globals.inbox.id })
			.then((response: InboxChannels) => {
				response._pagination.should.exist;
				response._pagination.should.include.keys('next', 'prev');
				response._links.should.exist;
				response._links.self.should.startsWith(
					`${apiUrl}/inboxes/${this.globals.inbox.id}/channels`,
				);
	...
	it('should list all conversations in the test inbox, finding previously known ones', function () {
		return this.globals.front.inbox
			.listConversations({ inbox_id: this.globals.inbox.id })
			.then((response: InboxConversations) => {
				response._pagination.should.exist;
				response._pagination.should.include.keys('next', 'prev');
				response._links.should.exist;
				response._links.self.should.startsWith(
					`${apiUrl}/inboxes/${this.globals.inbox.id}/conversations`,
				);
				response._results.should.exist;
	...
	it('should list all conversations in the test inbox using query string, finding previously known ones', function () {
		return this.globals.front.inbox
			...
			.then((response: InboxConversations) => {
				...
				response._pagination.should.include.keys('next', 'prev');
				response._links.should.exist;
				response._links.self.should.startWith(
					`${apiUrl}/inboxes/${this.globals.inbox.id}/conversations`,
				);
				response._links.self.should.include(
					'q[statuses][]=unassigned&q[statuses][]=assigned',
				);
				response._results.should.exist;
	...
	it('should list all teammates for the test inbox', function () {
		return this.globals.front.inbox
			.listTeammates({ inbox_id: this.globals.inbox.id })
			.then((response: InboxTeammates) => {
				response._pagination.should.exist;
				response._pagination.should.include.keys('next', 'prev');
				response._links.should.exist;
				response._links.self.should.startWith(
					`${apiUrl}/inboxes/${this.globals.inbox.id}/teammates`,
				);
				response._results.should.exist;
				response._results.length.should.be.gte(1);

				const testAuthor: Author | undefined = _.find(response._results, [
					'username',
					this.globals.author.username,
				]);
				if (!testAuthor) {
